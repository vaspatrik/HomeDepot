...
print 'Reading data'
df_train = pd.read_csv('train.csv', encoding="ISO-8859-1")[:10]#update here
df_test = pd.read_csv('test.csv', encoding="ISO-8859-1")[:10] #update here
df_pro_desc = pd.read_csv('product_descriptions.csv') #update here
df_attr = pd.read_csv('attributes.csv')#[:10000]
...
class ColumTextSelector(BaseEstimator, TransformerMixin):
	...
	def fit(self, x, y=None):
		return self
	def transform(self, full_df):
		return full_df[self.key].apply(str)
...
clf = pipeline.Pipeline([
		('union', FeatureUnion(
				transformer_list = [
					('txt1', pipeline.Pipeline([('s1', ColumTextSelector(key='search_term')), ('tfidf1', TfidfVectorizer(ngram_range=(1, 1), stop_words='english'))])),
					('txt2', pipeline.Pipeline([('s2', ColumTextSelector(key='product_title')), ('tfidf2', TfidfVectorizer(ngram_range=(1, 1), stop_words='english'))])),
					('txt3', pipeline.Pipeline([('s3', ColumTextSelector(key='product_description')), ('tfidf3', TfidfVectorizer(ngram_range=(1, 1), stop_words='english'))])),
					('txt4', pipeline.Pipeline([('s4', ColumTextSelector(key='brand')),('tfidf4', TfidfVectorizer(ngram_range=(1, 1), stop_words='english'))])),
					('txt5', pipeline.Pipeline([('s5', ColumTextSelector(key='bullet')),('tfidf5', TfidfVectorizer(ngram_range=(1, 1), stop_words='english'))])),
					('txt6', pipeline.Pipeline([('s6', ColumTextSelector(key='product_title_pps')), ('gramvec1', CountVectorizer(ngram_range=(1, 5)))])),
					('txt7', pipeline.Pipeline([('s7', ColumTextSelector(key='search_term_pps')), ('gramvec2', CountVectorizer(ngram_range=(1, 5)))])),
					('txt8', pipeline.Pipeline([('s8', ColumTextSelector(key='product_description_pps')), ('gramvec3',CountVectorizer(ngram_range=(1, 5)))])),
					('txt9', pipeline.Pipeline([('s9', ColumTextSelector(key='brand_pps')), ('gramvec4', CountVectorizer(ngram_range=(1, 5)))])),
					('txt10', pipeline.Pipeline([('s10', ColumTextSelector(key='bullet_pps')), ('gramvec5', CountVectorizer(ngram_range=(1, 5)))]))
