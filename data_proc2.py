...
df_prod = pd.read_csv('product_descriptions.csv').fillna(" ")
df_attr = pd.read_csv('attributes.csv').fillna(" ")
...
for i in range(len(df_all)):
    ...
    if b_ not in d_prod_query:
        d_prod_query[b_] = [list(set(unicode(df_all['search_term'][i]).lower().split(" "))), 
                            unicode(df_all['product_title'][i]).lower(),
                            unicode(df_prod.loc[df_prod['product_uid'] == df_all['product_uid'][i]]['product_description'].iloc[0]).lower()]
    else:
